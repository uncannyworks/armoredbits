...
-- | 'GameEvent's are events generated by the actual game simulation.
--
-- These message ids are odd.
data GameEvent
  -- | State of the Server and World
  = EventGameState ServerState WorldState
  -- | Map info, etc.
  | EventGameInfo
  -- | New client joined
  | EventLogin
...
instance Serialise GameEvent where
  encode = encodeGameEvent
  decode = decodeGameEvent

encodeGameEvent :: GameEvent -> Encoding
encodeGameEvent (EventGameState ss ws) =
  encodeListLen 3 <> encodeWord 1 <> encode ss <> encode ws
encodeGameEvent EventGameInfo =
  encodeListLen 1 <> encodeWord 3
encodeGameEvent EventLogin =
  encodeListLen 1 <> encodeWord 5
encodeGameEvent (EventSpawn e p r) =
  encodeListLen 4 <> encodeWord 7 <> encode e <> encode p <> encode r
encodeGameEvent (EventUpdate e p r) =
  encodeListLen 4 <> encodeWord 9 <> encode e <> encode p <> encode r
encodeGameEvent EventUnknown = mempty

decodeGameEvent :: Decoder s GameEvent
decodeGameEvent = do
  len <- decodeListLen
  tag <- decodeWord
  case (len, tag) of
    (3, 1) -> EventGameState <$> decode <*> decode
    (1, 3) -> pure EventGameInfo
    (1, 5) -> pure EventLogin
...
encodeServerMessage InvalidToken =
  encodeListLen 1 <> encodeWord 5
encodeServerMessage (EventGame e) =
...
data CMessage
  ...
  | SendToken Token
  -- | Meaningless message for testing
  | CTest
...
encodeClientMessage CTest =
  encodeListLen 1 <> encodeWord 1000
